...
async def broadcast(interaction: discord.Interaction, message: str, mention: str = "none"):
    ...
    embed.set_footer(text=f"📢 Sent by {interaction.user.top_role.name}")
...
async def safemode(interaction: discord.Interaction, password: str):
    ...
    embed = discord.Embed(title="✅ Safe Mode", description="Roles successfully given.", color=0x00cc00)
    embed.add_field(name="Roles", value=f"{role1.mention} {role2.mention}", inline=True)
    await interaction.response.send_message(embed=embed, ephemeral=True)
